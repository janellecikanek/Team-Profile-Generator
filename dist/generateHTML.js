...
const generateHTML = function(data) {
  ...
  for (let i = 0; i < data.length; i++) {
      ...
      if (role === 'Manager') {
          const managersSection = managerSection(employee);
          pageArray.push(managersSection);
      }

      // Calling engineersCard
      if (role === 'Engineer') {
          const engineersSection = engineerSection(employee);
          pageArray.push(engineersSection);
      }

      // Calling internsCard 
      if (role === 'Intern') {
          const internsSection = internSection(employee);
          pageArray.push(internsSection);
      }
